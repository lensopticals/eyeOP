...
const addProduct = (async (req, res) => {
  ...
  try {
    ...
    if (!saveProduct) {
      return res.status(202).json({success: false, message: "product doesn't save"});
    }
        res.json({success: true, product});
  } catch (error) {
    return res
      .status(500)
      .json({ success: true, message: "Internal server error", error: error.message });
  }
});
...
const deleteProduct = (async (req, res) => {
  try {
    ...
    if (!deleteproduct) {
      return res.status(500).json({success: false, message: "your product is not deleted."});
    }
    res.status(200).json({success: true, product});
  } catch (error) {
    console.error(error);
